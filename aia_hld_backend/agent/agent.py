...
def _clear_stale_research_validation_error_if_research_complete(state: dict) -> Optional[dict]:
    ...
    This handles resumed/looping sessions where:
    - research_resolved=True
    - technical_research_summary exists
    - resolved_services exists
    - but validation_error still contains old malformed research tool-call text.
    ...
    Important:
    - The malformed store_in_state error belongs to ResearchSubAgent state-writing.
    - It must not keep architecture_valid=False.
    - It must not trigger architect section-only retry.
    - It must not send the workflow back to intake.
    """
    if not isinstance(state, dict):
        return None

    # Only clear stale error if research is genuinely complete.
    # This protects normal incomplete research retries.
    if not _is_research_complete(state):
        return None
    ...
    validation_error_lower = validation_error_text.lower()

    is_research_malformed_error = (
        "malformed function call" in validation_error_lower
        or "malformed_function_call" in validation_error_lower
        or "print(default_api.store_in_state" in validation_error_lower
        or "default_api.store_in_state" in validation_error_lower
        or "store_in_state" in validation_error_lower
    ) and (
        "technical_research_summary" in validation_error_lower
        or "resolved_services" in validation_error_lower
        or "research" in validation_error_lower
        or str(KEY_TECHNICAL_RESEARCH_SUMMARY).lower() in validation_error_lower
        or str(KEY_RESOLVED_SERVICES).lower() in validation_error_lower
    )
    ...
    logger.warning(
        "[FLOW] Stale malformed research validation_error detected even though "
        "durable research state is complete. Clearing research error and unblocking architecture. "
        "validation_error_preview=%s",
        validation_error_text[:1000],
    )

    return {
        # Keep/force research complete.
        KEY_RESEARCH_RESOLVED: True,
        "research_resolved": True,

        # Reset research retry tracking.
        KEY_RESEARCH_RETRY_COUNT: 0,
        KEY_RESEARCH_RETRY_REASON: None,

        # Critical: clear stale malformed research error.
        KEY_VALIDATION_ERROR: None,

        # Critical: stale research error must not keep architecture invalid.
        # Do NOT mark architecture complete here.
        KEY_ARCH_VALID: True,

        # Preserve existing architecture completion only if already true.
        # Otherwise keep it false so architect loop can run normally.
        KEY_ARCH_COMPLETE: bool(state.get(KEY_ARCH_COMPLETE) is True),

        # Clear section retry controls if they were created only because of stale research error.
        ...
        while True:
            ...
            if not state.get(KEY_BLUEPRINT_SEARCH_DONE):
                ...
                if validation_error:
                    ...
                    is_intake_validation_error = (
                        "intake" in validation_error_text
                        or "missing required intake" in validation_error_text
                        or "intake validation" in validation_error_text
                        or str(KEY_INTAKE_VALIDATION_ERROR).lower() in validation_error_text
                    )

                    if is_intake_validation_error:
                        logger.warning(
                            "[FLOW] Blueprint blocked by intake validation error. "
                            "Returning to intake. validation_error=%s",
                            validation_error,
                        )
                        ...
                        _apply_state_delta_to_session(context, intake_reset_delta)
                        ...
                        yield Event(
                            ...
                            actions=EventActions(
                                state_delta=intake_reset_delta,
                                escalate=False,
                            ),
                        )
                    ...
                    logger.warning(
                        "[FLOW] Non-intake validation_error found before blueprint. "
                        "Clearing stale error instead of resetting intake. validation_error=%s",
                        validation_error,
                    )

                    stale_error_cleanup_delta = {
                        KEY_VALIDATION_ERROR: None,
                    }

                    _apply_state_delta_to_session(context, stale_error_cleanup_delta)
                    ...
                    yield Event(
                        ...
                        actions=EventActions(
                            state_delta=stale_error_cleanup_delta,
                            escalate=False,
                        ),
                    )
            ...
            if not state.get(KEY_RESEARCH_RESOLVED):
                ...
                if research_complete:
                    ...
                    research_success_delta = {
                        ...
                        # Important:
                        # Research completion should not leave architecture invalid
                        # because of a stale research malformed tool-call issue.
                        #
                        # Do NOT mark architecture complete here.
                        # Architect loop still needs to generate/validate HLD.
                        KEY_ARCH_VALID: True,
