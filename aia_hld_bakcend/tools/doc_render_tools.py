...
#         except Exception:
#             logger.exception("Graphviz render failed on first attempt. Retrying after edge-operator sanitization.")
...
#             png_path = src.render(filename="diagram_retry", directory=tmp, cleanup=True)
...
#             debug_path = debug_dir / f"failed_diagram_{hashlib.sha1(str(text).encode('utf-8')).hexdigest()[:12]}.dot"
#             debug_path.write_text(str(text), encoding="utf-8")

#             logger.error("Failed DOT source written to: %s", debug_path)
#         except Exception:
#             logger.exception("Could not write failed DOT debug file.")
